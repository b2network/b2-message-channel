...
import (
	...
	"bsquared.network/b2-message-channel-listener/internal/models"
	msg "bsquared.network/b2-message-channel-listener/internal/utils/ethereum/message"
	"bsquared.network/b2-message-channel-listener/internal/utils/log"
	"bytes"
	"context"
	"crypto/ecdsa"
	"encoding/hex"
...
func NewBuilder(key string, logConfig config.LogConfig, config config.Blockchain, db *gorm.DB, rpc *ethclient.Client) *Builder {
	return &Builder{
		db:     db,
		rpc:    rpc,
		config: config,
...
func (b *Builder) build() {
	duration := time.Millisecond * time.Duration(b.config.BlockInterval)
	for {
		list, err := b.pendingCallMessage(10)
		if err != nil {
...
func (b *Builder) buildMessage(message models.Message) error {
	...
	toAddress := common.HexToAddress(b.config.ListenAddress)
	...
	b.logger.Infof("FromChainId: %s,FromId: %s, FromSender: %s, ToContractAddress: %s, ToBytes: %s", message.FromChainId, common.HexToHash(message.FromId).Big().Text(10), message.FromSender, message.ToContractAddress, message.ToBytes)
	...
	err = b.db.Transaction(func(tx *gorm.DB) error {
		...
		// signTx
		_signature, err := b.SignTx(UserAddress, UserKey, nonce, toAddress.Hex(), big.NewInt(0), gasLimit, gasPrice, data, b.config.ChainId)
		if err != nil {
		...
		if err != nil {
			b.logger.Errorf("broadcast approve err: %s", err)
			return errors.WithStack(err)
		}
		b.logger.Infof("broadcast approve success")
		return nil
	})
	if err != nil {
...
func (b *Builder) pendingCallMessage(limit int) ([]models.Message, error) {
	var list []models.Message
	err := b.db.Where("`to_chain_id`=? AND `type`=? AND `status`=?", b.config.ChainId, enums.MessageTypeCall, enums.MessageStatusPending).Limit(limit).Find(&list).Error
	if err != nil {
		return nil, err
	}
	return list, nil
...
func (b *Builder) SignTx(accountAddress string, accountKey *ecdsa.PrivateKey, nonce uint64, toAddress string, value *big.Int, gasLimit uint64, gasPrice *big.Int, bytecode []byte, chainID int64) ([]byte, error) {
	//senderKey, err := b.getKeyByAddress(accountAddress)
	//if err != nil {
...
func (b *Builder) BorrowAccount() (string, *ecdsa.PrivateKey, error) {
	if b.mu.TryLock() {
		_key, err := crypto.ToECDSA(common.FromHex(b.key))
		if err != nil {
			return "", nil, err
		}
...
func (b *Builder) broadcast() {
	...
	for {
		...
		err := b.db.Where("`chain_id`=? AND `status`=?", b.config.ChainId, enums.SignatureStatusPending).Order("id").Limit(100).Find(&signatures).Error
		if err != nil {
			time.Sleep(duration)
			continue
		}
