...
import (
	...
	"bsquared.network/b2-message-channel-listener/internal/types"
	"bsquared.network/b2-message-channel-listener/internal/utils/aa"
...
func NewListener(logConfig config.LogConfig, config config.Blockchain, particle config.Particle, rpc *rpcclient.Client, db *gorm.DB) *BitcoinListener {
...
func (l *BitcoinListener) Name() string {
...
func (l *BitcoinListener) syncLatestBlock() {
	for {
		duration := time.Millisecond * time.Duration(l.config.BlockInterval) * 10
		...
		l.latestBlock = int64(latest) - l.config.SafeBlockNumber
...
func (l *BitcoinListener) syncTask() {
	for {
		duration := time.Millisecond * time.Duration(l.config.BlockInterval)
		var tasks []models.SyncTask
		err := l.db.Where("`chain_type`=? AND `chain_id`=? AND `status`=?", enums.ChainTypeUTXO, l.config.ChainId, enums.TaskStatusPending).Limit(20).Find(&tasks).Error
		if err != nil {
...
func (l *BitcoinListener) parseTx(txResult *wire.MsgTx, index int) (*types.BitcoinTxParseResult, error) {
	...
	for _, v := range txResult.TxOut {
		...
		_listenAddress, err := btcutil.DecodeAddress(l.config.ListenAddress, chainParams)
...
func (l *BitcoinListener) handleMessage(deposit models.Deposit) error {
	toChainId := l.config.ToChainId
	toContractAddress := l.config.ToContractAddress
