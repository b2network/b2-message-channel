...
import (
	...
	"bsquared.network/b2-message-channel-listener/internal/utils/ethereum/event"
	"bsquared.network/b2-message-channel-listener/internal/utils/ethereum/event/message"
...
func NewListener(logConfig config.LogConfig, config config.Blockchain, rpc *ethclient.Client, db *gorm.DB) *EthereumListener {
...
func (l *EthereumListener) Name() string {
...
func (l *EthereumListener) syncTask() {
	for {
		...
		err := l.db.Where("`chain_type`=? AND chain_id=? AND status=?", enums.ChainTypeEVM, l.config.ChainId, enums.TaskStatusPending).Limit(20).Find(&tasks).Error
		if err != nil {
...
func (l *EthereumListener) handleTask(task models.SyncTask) error {
	...
	logs, err := l.rpc.FilterLogs(context.Background(), ethereum.FilterQuery{
		...
		Addresses: []common.Address{
			common.HexToAddress(l.config.ListenAddress),
		},
	})
...
func (l *EthereumListener) pendingSendMessage(limit int) ([]models.Message, error) {
	...
	err := l.db.Where("`to_chain_id`=? AND `type`=? AND status=?", l.config.ChainId, enums.MessageTypeSend, enums.MessageStatusPending).Limit(limit).Find(&list).Error
	if err != nil {
		return nil, err
	}
	return list, nil
